...
(function(e) {
          ...
          if (b = m.config !== p && m.config.split ? !0 : !1, Array.isArray(m)) this._expandButton(a, m, g !== p && g.conf !== p ? g.conf.split : p, d, k !== p && k.split !== p, f, k);
            ...
            if (g) {
              ...
              if (g.conf.buttons || g.conf.split) {
                ...
                if (g.conf.split)
                  for (var t = 0; t < g.conf.split.length; t++) "object" === typeof g.conf.split[t] &&
                    (g.conf.split[c].parent = k, g.conf.split[t].collectionLayout === p && (g.conf.split[t].collectionLayout = g.conf.collectionLayout), g.conf.split[t].dropup === p && (g.conf.split[t].dropup = g.conf.dropup), g.conf.split[t].fade === p && (g.conf.split[t].fade = g.conf.fade));
      ...
      if ("absolute" === d) {
        ...
        if (c.hasClass(f.rightAlignClassName) || c.hasClass(f.leftAlignClassName) || c.hasClass(f.splitAlignClass) || "dt-container" === f.align) {
          ...
          if (c.hasClass(f.rightAlignClassName)) g =
            m - r, d > q + g && (d -= q + g, n -= r + g, g = d > n ? g + n : g + d);
  ...
  v.buttonSelector = function(a, b) {
    for (var c = [], d = function(h, n, q) {
        ...
        if (Array.isArray(h) || h instanceof e)
          for (m = 0, q = h.length; m < q; m++) l(h[m],
            n);
        else if (null === h || h === p || "*" === h)
          for (m = 0, q = r.length; m < q; m++) c.push({
            inst: n,
            node: r[m].node
          });
          ...
          if (-1 !== h.indexOf(","))
            for (r = h.split(","), m = 0, q = r.length; m < q; m++) l(r[m].trim(), n);
          else if (h.match(/^\d+(\-\d+)*$/)) m = e.map(r, function(g) {
          return g.idx
        }), c.push({
          inst: n,
          ...
          for (h = h.replace(":name", ""), m = 0, q = r.length; m < q; m++) r[m].name === h && c.push({
            inst: n,
            node: r[m].node
          });
  ...
  var K = function(a) {
    ...
    Q = function(a, b) {
      ...
      a.select && "function" === typeof a.select.info && l.selected === p && a.rows(c.rows, e.extend({
        selected: !0
      }, l)).any() && e.extend(l, {
        selected: !0
      });
